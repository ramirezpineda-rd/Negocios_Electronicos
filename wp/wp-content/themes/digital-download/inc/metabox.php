...
function digital_download_save_sidebar_layout( $post_id ){
    global $digital_download_sidebar_layout , $post;
    ...
    foreach( $digital_download_sidebar_layout as $field ){  
        ...
        $old = get_post_meta( $post_id, '_digital_download_sidebar_layout', true ); 
        $new = sanitize_text_field( $_POST['digital_download_sidebar_layout'] );
        if( $new && $new != $old ) {  
            update_post_meta( $post_id, '_digital_download_sidebar_layout', $new );  
        }elseif( '' == $new && $old ) {  
            delete_post_meta( $post_id, '_digital_download_sidebar_layout', $old );  
        } 
     } // end foreach     
}
